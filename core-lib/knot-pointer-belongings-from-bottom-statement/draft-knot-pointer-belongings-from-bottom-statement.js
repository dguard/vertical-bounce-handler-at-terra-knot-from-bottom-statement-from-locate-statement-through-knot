...
let throughY
let fromDeTry
let knotDe = originalSpawnedPointerBelongings.knotRelativeSizeAtDebth / originalSpawnedPointerBelongings.knotAtDistanceY / 4
...
for(fromDeTry = 1; fromDeTry < iterationCountAtDe; fromDeTry++) {
    // use descending-from-the-sun, accurate-pencil
    if(spawnedPointerBelongingsFromBottomStatement.y + (fromDeTry * knotDe) < originalSpawnedPointerBelongings.throughY
        && originalSpawnedPointerBelongings.throughY < (spawnedPointerBelongingsFromBottomStatement.y + /* keep fromDeTry + knotDe */ ((fromDeTry + 1) * knotDe))) {
            throughY = spawnedPointerBelongingsFromBottomStatement.y + fromDeTry * knotDe
            break
    }
}
let increaserDe = (knotDe / originalSpawnedPointerBelongings.knotRelativeSizeAtDebth) * fromDeTry
